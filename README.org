...
** App Downloads
:PROPERTIES:
:CUSTOM_ID: app-downloads
:END:
On Android, add the Mova F-Droid repository to receive updates automatically:

- https://colonelpanic8.github.io/mova/

The phone and Wear OS APKs are also attached to each GitHub release:

- https://github.com/colonelpanic8/mova/releases/latest

The F-Droid repository serves the same signed phone APKs as the releases page,
so installs from either source upgrade in place. See
[[file:docs/fdroid.md][docs/fdroid.md]] for publication and repository setup details.
...
- Agenda widget listing today's items (overdue first), each with a
  tap-to-complete circle that finishes the item without opening the app, plus
  task/habit tabs and typed and voice quick-capture actions in the header;
  scheduled/deadline timestamps appear on cards, with two cards per row at
  wider sizes

*** Intents API (Android)
:PROPERTIES:
:CUSTOM_ID: intents-api
:END:
- =mova://= links let any app or assistant create, complete, update, delete
  and open todos, run a search, jump to a date, or open the capture dialogs
- A read-only content provider exposes todo search and the agenda to apps
  that hold the =READ_TODOS= runtime permission
- Every action, parameter and value format is documented in
  [[file:docs/intents.md][docs/intents.md]]
...
- *TanStack Query* - Server-state caching, invalidation, and offline
  persistence (queries are keyed per server identity and persisted to
  AsyncStorage for offline launches)
- *TypeScript* - Type safety
- *Jest* + *Detox* - Testing

Note: the home-screen widget, quick-capture activity, and Wear OS
companion app are Android-only; iOS currently has no counterpart for
these capture surfaces.
...
*** Watch Assistant
:PROPERTIES:
:CUSTOM_ID: watch-assistant
:END:

To enable natural-language requests from Wear OS:

1. Open Mova's Settings screen on the phone.
2. Under =Watch Assistant=, enter an OpenAI API key and model.
3. Tap =Save= while the watch is paired.
...
The assistant can search and count todos, inspect agendas and habits, capture,
update, complete, or explicitly delete tasks, and use other allowlisted API
operations. It resolves mutation targets to an org ID or exact file position
before acting and asks for clarification rather than guessing when a target is
ambiguous.
